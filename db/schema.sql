...
CREATE TABLE IF NOT EXISTS `magnolia`.`publication` (
  ...
  `reqn_id` INT(10) UNSIGNED NOT NULL,
  `filename` VARCHAR(255) NOT NULL,
  `reference` VARCHAR(512) NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
...
CREATE TABLE IF NOT EXISTS `magnolia`.`reqn` (
  ...
  `deferral_note_2g` TEXT NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `magnolia`.`reqn_version` (
  ...
  `applicant_country_id` INT UNSIGNED NULL DEFAULT NULL,
  ...
  `trainee_country_id` INT UNSIGNED NULL DEFAULT NULL,
...
-- -----------------------------------------------------
-- Table `magnolia`.`user_has_graduate`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `magnolia`.`user_has_graduate` (
  `user_id` INT(10) UNSIGNED NOT NULL,
  `graduate_user_id` INT(10) UNSIGNED NOT NULL,
  `update_timestamp` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP(),
  `create_timestamp` TIMESTAMP NOT NULL DEFAULT '0000-00-00 00:00:00',
...
DELIMITER $$
USE `cenozo`$$
CREATE DEFINER = CURRENT_USER TRIGGER `cenozo`.`address_BEFORE_INSERT` BEFORE INSERT ON `cenozo`.`address` FOR EACH ROW
BEGIN
...
USE `cenozo`$$
CREATE DEFINER = CURRENT_USER TRIGGER `cenozo`.`address_AFTER_INSERT` AFTER INSERT ON `cenozo`.`address` FOR EACH ROW
BEGIN
...
USE `cenozo`$$
CREATE DEFINER = CURRENT_USER TRIGGER `cenozo`.`address_BEFORE_UPDATE` BEFORE UPDATE ON `cenozo`.`address` FOR EACH ROW
BEGIN
  IF ( NEW.alternate_id IS NULL AND NEW.participant_id IS NULL ) or
     ( NEW.alternate_id IS NOT NULL AND NEW.participant_id IS NOT NULL ) THEN
...
USE `cenozo`$$
CREATE DEFINER = CURRENT_USER TRIGGER `cenozo`.`address_AFTER_UPDATE` AFTER UPDATE ON `cenozo`.`address` FOR EACH ROW
BEGIN
  IF NEW.alternate_id IS NOT NULL THEN
    CALL update_alternate_first_address( NEW.alternate_id );
  ELSE
...
USE `cenozo`$$
CREATE DEFINER = CURRENT_USER TRIGGER `cenozo`.`address_AFTER_DELETE` AFTER DELETE ON `cenozo`.`address` FOR EACH ROW
BEGIN
...
USE `cenozo`$$
CREATE DEFINER = CURRENT_USER TRIGGER `cenozo`.`event_type_AFTER_INSERT` AFTER INSERT ON `cenozo`.`event_type` FOR EACH ROW
BEGIN
...
USE `cenozo`$$
CREATE DEFINER = CURRENT_USER TRIGGER `cenozo`.`hin_AFTER_INSERT` AFTER INSERT ON `cenozo`.`hin` FOR EACH ROW
BEGIN
  CALL update_participant_last_hin( NEW.participant_id );
END$$

USE `cenozo`$$
CREATE DEFINER = CURRENT_USER TRIGGER `cenozo`.`hin_AFTER_UPDATE` AFTER UPDATE ON `cenozo`.`hin` FOR EACH ROW
BEGIN
  CALL update_participant_last_hin( NEW.participant_id );
END$$

USE `cenozo`$$
CREATE DEFINER = CURRENT_USER TRIGGER `cenozo`.`hin_AFTER_DELETE` AFTER DELETE ON `cenozo`.`hin` FOR EACH ROW
BEGIN
...
DELIMITER $$
USE `magnolia`$$
CREATE DEFINER = CURRENT_USER TRIGGER `magnolia`.`data_category_AFTER_INSERT` AFTER INSERT ON `data_category` FOR EACH ROW
BEGIN
  IF NEW.comment THEN
...
USE `magnolia`$$
CREATE DEFINER = CURRENT_USER TRIGGER `magnolia`.`data_category_BEFORE_UPDATE` BEFORE UPDATE ON `data_category` FOR EACH ROW
BEGIN
...
USE `magnolia`$$
CREATE DEFINER = CURRENT_USER TRIGGER `magnolia`.`final_report_AFTER_INSERT` AFTER INSERT ON `final_report` FOR EACH ROW
BEGIN
  CALL update_reqn_current_final_report( NEW.reqn_id );
END$$

USE `magnolia`$$
CREATE DEFINER = CURRENT_USER TRIGGER `magnolia`.`final_report_AFTER_DELETE` AFTER DELETE ON `final_report` FOR EACH ROW
BEGIN
...
USE `magnolia`$$
CREATE DEFINER = CURRENT_USER TRIGGER `magnolia`.`reqn_version_AFTER_UPDATE` AFTER UPDATE ON `reqn_version` FOR EACH ROW
BEGIN
...
USE `magnolia`$$
CREATE DEFINER = CURRENT_USER TRIGGER `magnolia`.`reqn_version_has_amendment_type_AFTER_INSERT` AFTER INSERT ON `reqn_version_has_amendment_type` FOR EACH ROW
BEGIN
...
USE `magnolia`$$
CREATE DEFINER = CURRENT_USER TRIGGER `magnolia`.`reqn_version_has_amendment_type_AFTER_DELETE` AFTER DELETE ON `reqn_version_has_amendment_type` FOR EACH ROW
BEGIN
...
USE `magnolia`$$
CREATE DEFINER = CURRENT_USER TRIGGER `magnolia`.`reqn_version_has_data_selection_AFTER_INSERT` AFTER INSERT ON `reqn_version_has_data_selection` FOR EACH ROW
BEGIN
...
USE `magnolia`$$
CREATE DEFINER = CURRENT_USER TRIGGER `magnolia`.`reqn_version_has_data_selection_AFTER_DELETE` AFTER DELETE ON `reqn_version_has_data_selection` FOR EACH ROW
BEGIN
...
USE `magnolia`$$
CREATE DEFINER = CURRENT_USER TRIGGER `magnolia`.`review_AFTER_INSERT` AFTER INSERT ON `review` FOR EACH ROW
BEGIN
...
USE `magnolia`$$
CREATE DEFINER = CURRENT_USER TRIGGER `magnolia`.`review_type_question_AFTER_INSERT` AFTER INSERT ON `review_type_question` FOR EACH ROW
BEGIN
  -- create all review_answer records
  INSERT INTO review_answer( review_id, review_type_question_id )
