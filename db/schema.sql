...
-- -----------------------------------------------------
-- Table `magnolia`.`data_option_category_has_study_phase`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `magnolia`.`data_option_category_has_study_phase` (
  `data_option_category_id` INT(10) UNSIGNED NOT NULL,
...
-- -----------------------------------------------------
-- Table `magnolia`.`data_option_justification`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `magnolia`.`data_option_justification` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `update_timestamp` TIMESTAMP NOT NULL,
  `create_timestamp` TIMESTAMP NOT NULL,
  `reqn_version_id` INT(10) UNSIGNED NOT NULL,
  ...
  CONSTRAINT `fk_data_option_justification_reqn_version_id`
    FOREIGN KEY (`reqn_version_id`)
    REFERENCES `magnolia`.`reqn_version` (`id`)
    ON DELETE CASCADE
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_data_option_justification_data_option_id`
...
-- -----------------------------------------------------
-- Table `magnolia`.`data_selection_detail`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `magnolia`.`data_selection_detail` (
  `id` INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `update_timestamp` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP() ON UPDATE CURRENT_TIMESTAMP(),
  `create_timestamp` TIMESTAMP NOT NULL DEFAULT '0000-00-00 00:00:00',
...
DELIMITER $$
USE `magnolia`$$
CREATE DEFINER = CURRENT_USER TRIGGER `magnolia`.`data_option_category_AFTER_INSERT` AFTER INSERT ON `data_option_category` FOR EACH ROW
BEGIN
  IF NEW.comment THEN
...
USE `magnolia`$$
CREATE DEFINER = CURRENT_USER TRIGGER `magnolia`.`data_option_category_BEFORE_UPDATE` BEFORE UPDATE ON `data_option_category` FOR EACH ROW
BEGIN
  IF NEW.comment != OLD.comment THEN
    IF NEW.comment THEN
      -- create corresponding reqn_version_comment records
      INSERT IGNORE INTO reqn_version_comment( create_timestamp, reqn_version_id, data_option_category_id )
      SELECT NEW.create_timestamp, reqn_version.id, NEW.id
      FROM reqn_version;
    ELSE
      -- remove corresponding reqn_version_comment records (keep filled out records)
      DELETE FROM reqn_version_comment WHERE data_option_category_id = NEW.id AND comment IS NULL;
    END IF;
  END IF;
END$$
