...
CREATE TABLE IF NOT EXISTS `magnolia`.`amendment_type` (
  ...
  `fee_canada` INT UNSIGNED NULL DEFAULT NULL,
  `fee_international` INT UNSIGNED NULL DEFAULT NULL,
...
USE `magnolia`$$
CREATE DEFINER = CURRENT_USER TRIGGER `magnolia`.`reqn_version_BEFORE_UPDATE` BEFORE UPDATE ON `reqn_version` FOR EACH ROW
BEGIN
