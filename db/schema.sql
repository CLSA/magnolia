...
USE `magnolia`$$
CREATE DEFINER = CURRENT_USER TRIGGER `magnolia`.`reqn_version_data_option_AFTER_INSERT` AFTER INSERT ON `reqn_version_data_option` FOR EACH ROW
BEGIN
...
USE `magnolia`$$
CREATE DEFINER = CURRENT_USER TRIGGER `magnolia`.`reqn_version_data_option_AFTER_DELETE` AFTER DELETE ON `reqn_version_data_option` FOR EACH ROW
BEGIN
