...
CREATE PROCEDURE patch_role_has_service()
  BEGIN
    ...
    SET @sql = CONCAT(
      ...
      "WHERE role.name IN( 'administrator', 'applicant' ) ",
      "AND service.subject = 'ethics_approval' ",
    ...
    SET @sql = CONCAT(
      ...
      "AND service.subject IN( 'output_type', 'pdf_form' ) ",
      "AND service.restricted = 1"
    );
    PREPARE statement FROM @sql;
    EXECUTE statement;
    ...
    SET @sql = CONCAT(
      ...
      "SELECT role.id, service.id ",
      "FROM ", @cenozo, ".role, service ",
      "WHERE role.name IN( 'administrator', 'applicant', 'typist' ) ",
      "AND service.subject = 'output_source' ",
      "AND service.restricted = 1"
    );
    PREPARE statement FROM @sql;
    EXECUTE statement;
