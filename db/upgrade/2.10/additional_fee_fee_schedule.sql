DROP PROCEDURE IF EXISTS patch_additional_fee_fee_schedule;
DELIMITER //
CREATE PROCEDURE patch_additional_fee_fee_schedule()
  BEGIN

    SELECT "Creating new additional_fee_fee_schedule table" AS "";

    CREATE TABLE IF NOT EXISTS additional_fee_fee_schedule (
    ...
    SELECT COUNT(*) INTO @test FROM additional_fee_fee_schedule;
    IF @test = 0 THEN
      INSERT INTO additional_fee_fee_schedule(additional_fee_id, fee_schedule_id, fee)
      SELECT additional_fee.id, fee_schedule.id, cost
      FROM additional_fee, fee_schedule;
    END IF;

  END //
DELIMITER ;

CALL patch_additional_fee_fee_schedule();
DROP PROCEDURE IF EXISTS patch_additional_fee_fee_schedule;
