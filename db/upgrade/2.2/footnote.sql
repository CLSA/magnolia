...
CREATE PROCEDURE patch_footnote()
  BEGIN
    ...
    IF @test = 0 THEN
      ...
      ( "Raw data are available by special request. To request raw data, please go to the Additional Deta section.", "Les données brutes sont disponibles sur demande spéciale. Pour demander les données brutes, visitez la section Données supplémentaires." ),
      ...
      ( "Please note that a request to receive genomics data may incur additional costs for transfer of the data, beyond the current data access fee. Shipping & associated fees to be determined.", "TRANSLATION REQUIRED" ),
      ...
      ( "Although MEDI data are not yet available, they may be requested in the Comments box below. If your project is approved, you will receive these data once they become available.", "TRANSLATION REQUIRED" ),
      ( "Electrocardiogram (ECG) tracings are available by special request. To request ECG tracings, please go to the Additional Deta section.", "Il est maintenant possible d’obtenir les tracés d’électrocardiogramme (ECG) sur demande spéciale. Pour demander les tracés d’ECG , visitez la section Données supplémentaires." ),
      ( "Spirometry (SPR) flow curves are available by special request. To request SPR flow curves, please go to the Additional Deta section.", "Il est maintenant possible d’obtenir les courbes de débit de spirométrie (SPR). Pour demander les courbes de débit de SPR, visitez la section Données supplémentaires." ),
      ( "Images and raw data are available by special request. To request images or raw data, please go to the Additional Deta section.", "Il est maintenant possible d’obtenir les données en format image et les données brutes sur demande spéciale. Pour demander des données en format images ou les données brutes, visitez la section Données supplémentaires." );
