...
CREATE PROCEDURE patch_role_has_service()
  BEGIN
    ...
    SET @sql = CONCAT(
      ...
      "AND service.subject IN( 'data_agreement', 'notation' ) ",
    ...
    SET @sql = CONCAT(
      ...
      "SELECT role.id, service.id ",
      "FROM ", @cenozo, ".role, service ",
      "WHERE role.name IN( 'administrator', 'applicant', 'typist', 'designate' ) ",
      "AND subject = 'coapplicant' ",
      "AND restricted = true"
    );
    PREPARE statement FROM @sql;
    EXECUTE statement;
    ...
    SET @sql = CONCAT(
      ...
      "AND service.subject IN( 'pdf_form', 'pdf_form_type' ) ",
      "AND service.restricted = 1"
    );
    PREPARE statement FROM @sql;
    EXECUTE statement;
