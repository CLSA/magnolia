cenozoApp.defineModule({
  ...
  create: (module) => {
      ...
      viewed_by_trainee_user: {
        ...
        isExcluded: function ($state, model) {
          return "add" == model.getActionFromState()
            ? true
            : null == model.viewModel.record.trainee_user_id;
        },
      },
      viewed_by_designate_user: {
        ...
        isExcluded: function ($state, model) {
          return "add" == model.getActionFromState()
            ? true
            : null == model.viewModel.record.designate_user_id;
        },
      },
      description: {
        title: "Description",
    ...
    /* ############################################################################################## */
    cenozo.providers.factory("CnManuscriptNoticeViewFactory", [
      "CnBaseViewFactory",
      "CnHttpFactory",
      function (CnBaseViewFactory, CnHttpFactory) {
        var object = function (parentModel, root) {
          ...
          this.onPatch = async function (data) {
            if (
              angular.isDefined(data.viewed_by_user) ||
              ...
              if (add) {
                await CnHttpFactory.instance({
                  path: this.parentModel.getServiceResourcePath() + "/user",
                  data: userId,
                }).post();
              } else {
                await CnHttpFactory.instance({
    ...
    /* ############################################################################################## */
    cenozo.providers.factory("CnManuscriptNoticeModelFactory", [
      "CnBaseModelFactory",
      "CnManuscriptNoticeAddFactory",
      "CnManuscriptNoticeListFactory",
      "CnManuscriptNoticeViewFactory",
      function (
        CnBaseModelFactory,
        CnManuscriptNoticeAddFactory,
        CnManuscriptNoticeListFactory,
        ...
        var object = function (root) {
          ...
          this.addModel = CnManuscriptNoticeAddFactory.instance(this);
          this.listModel = CnManuscriptNoticeListFactory.instance(this);
          this.viewModel = CnManuscriptNoticeViewFactory.instance(this, root);
