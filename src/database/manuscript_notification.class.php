<?php
/**
 * manuscript_notification.class.php
 * 
...
/**
 * manuscript_notification: record
 */
class manuscript_notification extends \cenozo\database\record
{
  /**
   * Adds an email address to the manuscript_notification
   * @param string $email
   * @param string $name
   */
  public function add_email( $email, $name )
  {
    // make sure the manuscript_notification recipient doesn't already exist
    $manuscript_notification_email_class_name = lib::get_class_name( 'database\manuscript_notification_email' );
    $db_manuscript_notification_email = $manuscript_notification_email_class_name::get_unique_record(
      array( 'manuscript_notification_id', 'email' ),
      array( $this->id, $email )
    );

    if( is_null( $db_manuscript_notification_email ) )
    {
      $db_manuscript_notification_email = lib::create( 'database\manuscript_notification_email' );
      $db_manuscript_notification_email->manuscript_notification_id = $this->id;
      $db_manuscript_notification_email->email = $email;
      $db_manuscript_notification_email->name = $name;
      $db_manuscript_notification_email->save();
    }
  }

  /**
   * Sets up the manuscript_notification to send to the owner, trainne and designate of a manuscript's reqn
   * @param database\manuscript $db_manuscript
   */
  public function set_manuscript( $db_manuscript )
  {
    ...
    $this->manuscript_id = $db_manuscript->id;
  ...
  /**
   * Sends all emails associated with this manuscript_notification
   */
  public function mail()
  {
    ...
    $db_reqn = $this->get_manuscript()->get_reqn();
  ...
  /**
   * Sends an email manuscript_notification to the admin_email (from general settings)
   */
